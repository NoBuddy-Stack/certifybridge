...
export const COLLECTION_AUDIT_LOG    = 'audit_log';
...
export async function ensureIndexes(col) {
  ...
  await Promise.all([
    // unique + sparse — matches the existing production index
    ...
    // Text index for admin search (replaces $regex COLLSCAN)
    col.createIndex(
      { firstName: 'text', lastName: 'text', email: 'text', college: 'text', razorpayOrderId: 'text' },
      { name: 'admin_search_text' }
    ),
  ]);
